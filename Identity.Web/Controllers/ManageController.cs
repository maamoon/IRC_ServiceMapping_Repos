...
namespace Identity.Web.Controllers
{
    ...
    public class ManageController : BaseController
    {
        ...
        public async Task<ActionResult> ChangePicturePost(HttpPostedFileBase DocFile )
        {
            if (DocFile != null)
            {
                try
                {
                    var attachedFile = DocFile;
                    string FilePath = FileHelper.SaveImage(attachedFile.InputStream, Request.Form["customFileName"]);
                    
                    var currentUser = await UserManager.FindByIdAsync(UserId);
                    currentUser.Profile.ProfilePicture = FilePath;
                    await UserManager.UpdateAsync(currentUser);
                    Session["CurrentUserPicture"] = FilePath;
                    return Json(new { filePath = FilePath, fileName = Request.Form["customFileName"] });
                }
                catch (Exception ex)
                {
                    throw ex;
