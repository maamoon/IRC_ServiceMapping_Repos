...
namespace Identity.Web.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        {
            var ServiceMappingList = UserManager.Users;
            try
            {


                var result = (from item in ServiceMappingList
                              select new object[]
                          {
                              item.Id,
                              item.UserName,
                              item.UserRole==null?"": item.UserRole.Name,
                          }).ToArray();
                var jsonData = new
                {
                    total = result.Count(),
        ...
        public ActionResult CreateNewUser()
        {
            ...
            return PartialView("UserModel", new IdentityUser());
        }
        [HttpGet]
        public async Task<ActionResult> ViewUser(Guid Userid = new Guid())
        {
            IdentityUser user = await UserManager.FindByIdAsync(Userid);
            ViewBag.IsViewMode = true;
            ViewBag.Title = "View User";
            return PartialView("UserModel", user);
        }
        [HttpPost]
        public async Task<ActionResult> DeleteUser(Guid Userid = new Guid())
        {
            try
            {
                if (Userid == UserId)
                ...
                case SignInStatus.Success:
                    {
                        ...
                        Session["CurrentUserPicture"] = currentUser.Profile.ProfilePicture;
